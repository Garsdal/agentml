# AGENTS.md — Dojo.ml Codebase Reference

> **Dojo.ml** — AI-powered ML experiment orchestration with a hexagonal (ports & adapters) architecture.
...
**Composition root:** `src/dojo/api/deps.py` → `build_lab()` dispatches on `settings.tracking.backend` and `settings.memory.backend`.

## Directory Map

| Directory | Purpose |
|---|---|
| `src/dojo/core/` | Pure domain: `Experiment`, `Task`, `KnowledgeAtom`, `ExperimentState` enum, state machine transitions |
| `src/dojo/interfaces/` | ABCs (ports): `Agent`, `ComputeBackend`, `Sandbox`, `ExperimentStore`, `ArtifactStore`, `MemoryStore`, `TrackingConnector`, `ToolRuntime` |
| `src/dojo/agents/` | Agent implementations. Currently: `StubAgent` (mock, no LLM) |
| `src/dojo/compute/` | Compute backends. Currently: `LocalCompute` (in-process) |
| `src/dojo/sandbox/` | Code execution. Currently: `LocalSandbox` (subprocess) |
| `src/dojo/storage/` | Persistence adapters: `LocalExperimentStore` (JSON), `LocalArtifactStore` (files), `LocalMemoryStore` (JSON keyword search) |
| `src/dojo/tracking/` | Experiment tracking: `FileTracker` (JSON), `MlflowTracker` (MLflow ≥3.0), `NoopTracker` |
| `src/dojo/runtime/` | `LabEnvironment` (DI dataclass), `ExperimentService` (lifecycle orchestration) |
| `src/dojo/api/` | FastAPI app + routers. `app.py` creates app, `deps.py` builds lab |
| `src/dojo/cli/` | Typer CLI: `start`, `run`, `config init/show` |
| `src/dojo/config/` | `Settings` (pydantic-settings), `defaults.py`, YAML loading |
| `src/dojo/utils/` | `generate_id()` (ULID), JSON serialization, structlog setup |
...
YAML at `.dojo/config.yaml`, overridable via env vars.
...
**⚠️ Gotcha:** The pydantic-settings prefix is `DOJO_` (single trailing underscore), and nested fields are separated by `__` (double underscore). This means an env var has **one** `_` between `DOJO` and the top-level field, not two:

```
DOJO_<FIELD>__<SUBFIELD>=value
     ^      ^^
...
|---|---|
| `DOJO_AGENT__BACKEND=stub` | `DOJO__AGENT__BACKEND=stub` |
| `DOJO_TRACKING__BACKEND=mlflow` | `DOJO__TRACKING__BACKEND=mlflow` |
| `DOJO_TRACKING__MLFLOW_TRACKING_URI=...` | `DOJO__TRACKING__MLFLOW_TRACKING_URI=...` |
...
1. **Create adapter** in the matching directory (e.g., `src/dojo/storage/postgres_experiment.py`)
2. **Implement the interface** from `src/dojo/interfaces/`
3. **Wire in `build_lab()`** (`src/dojo/api/deps.py`) — add dispatch case for the new `backend` string
4. **Add config fields** if needed to `src/dojo/config/settings.py`
5. **Add tests** in `tests/unit/` (adapter round-trip) and `tests/integration/` (through LabEnvironment)

## Adding a New API Route

1. Create router in `src/dojo/api/routers/` with `APIRouter(prefix="/...", tags=[...])`
2. Access lab via `request.app.state.lab: LabEnvironment`
3. Register in `src/dojo/api/app.py`: `app.include_router(router)`
4. Add E2E test in `tests/e2e/test_full_lifecycle.py`
5. Add frontend hook in `frontend/src/hooks/` and page/component as needed

## Key Conventions

- **IDs**: ULIDs via `dojo.utils.ids.generate_id()`
